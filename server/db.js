...
await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS permissions TEXT DEFAULT '["metricas","rentabilidad","tiendas","productos","pedidos","facturas","informes","exprod","ayuda","reembolsos_widget"]'`);
// La columna ya existía en producción: ADD COLUMN IF NOT EXISTS no actualiza su DEFAULT, hace falta forzarlo
await pool.query(`ALTER TABLE users ALTER COLUMN permissions SET DEFAULT '["metricas","rentabilidad","tiendas","productos","pedidos","facturas","informes","exprod","ayuda","reembolsos_widget"]'`);
await pool.query(`
