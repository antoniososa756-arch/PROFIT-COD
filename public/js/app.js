        ...
        <tbody>
          ${items.map(item => {
            ...
            <tr data-id="${item.id}" onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
              ...
              <td style="padding:7px 12px;border:1px solid #374151;">
                <div onclick="abrirModalGFValor('${item.id}','${mes}',${parseFloat(item.valor)||0})"
      ...
      <table style="width:100%;border-collapse:collapse;font-size:13px;">
        ...
        <tbody>
          <tr onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
            ...
            <td style="padding:10px 14px;border:1px solid #374151;text-align:right;">
              <div onclick="abrirModalGFGlobal('mrw',${parseFloat(preciosGlobales.precio_mrw)||0})"
          ...
          <tr onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
            ...
            <td style="padding:10px 14px;border:1px solid #374151;text-align:right;">
              <div onclick="abrirModalGFGlobal('logistica',${parseFloat(preciosGlobales.precio_logistica)||0})"
...
async function _abrirModalGF(valorActual, label, onSaveFn) {
  ...
  document.body.appendChild(overlay);
  document.getElementById("mgf-save-btn").addEventListener("click", async () => {
    const sinIvaEl = document.getElementById("mgf-sin-iva");
    const valor = parseFloat(sinIvaEl?.value) || 0;
    await onSaveFn(valor);
    overlay.remove();
  });
...
window.abrirModalGFValor = abrirModalGFValor;
...
window.abrirModalGFPrecioUnit = abrirModalGFPrecioUnit;

async function abrirModalGFGlobal(campo, valorActual) {
...
window.abrirModalGFGlobal = abrirModalGFGlobal;
...
async function loadFiscalidadIva(forzarMonth, forzarYear) {
  ...
  const storeResults = stores.map(store => {
    ...
    // totalToggle: recargo activado (RE) o IVA deducible activado (SL)
    ...
    const rowsHtml = rows.map((r, i) => {
      const ivaVal = r.base * ivaFactor;
      if (!esSL_card) {
        // RE: IVA fijo + toggle de recargo (5.2%)
        const activeRec = !!toggles[i];
        const recVal    = activeRec ? r.base * toggleFactor : 0;
        return `
      <tr${i%2===1?' style="background:#1f2937;"':''}>
        ...
        <td style="padding:9px 14px;border:1px solid #374151;text-align:right;font-size:13px;">
          <div style="display:flex;align-items:center;justify-content:flex-end;gap:8px;">
            <span id="rec-val-${domainSlug}-${i}" style="font-weight:600;color:${activeRec?'#22c55e':'#4b5563'};">${activeRec ? fmt(recVal)+' €' : '—'}</span>
            <span id="rec-tog-${domainSlug}-${i}"
              onclick="toggleRecargoRow('${store.domain}',${i},${r.base})"
              title="${activeRec?'Quitar':'Activar'}"
              style="display:inline-block;width:14px;height:14px;border-radius:50%;background:${activeRec?'#22c55e':'#374151'};border:2px solid ${activeRec?'#16a34a':'#6b7280'};cursor:pointer;flex-shrink:0;transition:background .2s;">
            </span>
          </div>
        </td>
      </tr>`;
        ...
        <table style="width:100%;border-collapse:collapse;">
          <thead>
            <tr style="background:#1f2937;">
              ...
              <th style="padding:8px 14px;border:1px solid #374151;text-align:right;font-size:11px;color:#f59e0b;font-weight:600;">${esSL_card ? `IVA deducible (${ivaPct}%)` : `IVA (${ivaPct}%)`}</th>
              ${!esSL_card ? `<th style="padding:8px 14px;border:1px solid #374151;text-align:right;font-size:11px;color:#22c55e;font-weight:600;">Recargo (5.2%)</th>` : ''}
            </tr>
          </thead>
          <tbody>
            ${rowsHtml}
            <tr style="background:${esSL_card?'rgba(34,197,94,.1)':'rgba(245,158,11,.1)'};">
              ...
              <td style="padding:11px 14px;border:1px solid #374151;text-align:right;font-weight:700;color:${esSL_card?'#22c55e':'#f59e0b'};font-size:15px;" ${esSL_card?`id="total-rec-${domainSlug}"`:''}>${fmt(esSL_card ? totalToggle : totalIva)} €</td>
              ${!esSL_card ? `<td style="padding:11px 14px;border:1px solid #374151;text-align:right;font-weight:700;color:#22c55e;font-size:15px;" id="total-rec-${domainSlug}">${fmt(totalToggle)} €</td>` : ''}
            </tr>
          </tbody>
        </table>
      </div>
...
// Cargar pedidos del mes al cache global (solo el mes seleccionado)
  ...
  const cols = stores.map(store => {
    ...
    // Toggles SL (necesarios para display y cálculo de total)
    let togglesSL = {};
    if (esSL) { try { togglesSL = JSON.parse(localStorage.getItem(`sl_iva_toggles_${store.domain}`) || "{}"); } catch {} }
    // fmtT: aplica IVA solo si el toggle del índice está activo (SL), o no aplica (RE)
    const fmtT = (n, idx) => {
      const factor = (esSL && togglesSL[idx]) ? (1 + ivaPorcentaje) : 1;
      return (Math.round((parseFloat(n)||0) * factor * 100) / 100).toFixed(2);
    };
    ...
    if (esSL) {
      ...
      const rowsSL = [
        ads.meta, ads.tiktok, costoProductosNeto, mrw, logistica, fijoXTienda, shopify,
        ...(gastosExtras[store.domain]||[]).map(g => parseFloat(g.valor)||0),
      ];
      const ivaDeducibleSL = rowsSL.reduce((s,b,i) => s + (togglesSL[i] ? b * ivaPorcentaje : 0), 0);
      ivaSLNetoPagar = ivaRepercutidoSL - ivaDeducibleSL;
    }

    // Total: suma cada item con su factor de IVA según toggle + nómina + IVA neto
    const rowBasesGV = [ads.meta, ads.tiktok, costoProductosNeto, mrw, logistica, fijoXTienda, shopify,
      ...(gastosExtras[store.domain]||[]).map(g => parseFloat(g.valor)||0)];
    const total = esSL
      ? rowBasesGV.reduce((s,b,i) => s + b * (togglesSL[i] ? (1+ivaPorcentaje) : 1), 0) + nominaXTienda + ivaSLNetoPagar
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
          <tbody>
            <tr>
              ...
              <td style="padding:10px 14px;border:1px solid #374151;text-align:right;color:#6b7280;">${fmtT(ads.meta,0)} €</td>
            </tr>
            <tr style="background:#1f2937;">
              <td style="padding:10px 14px;border:1px solid #374151;font-weight:600;color:#e5e7eb;">Gasto TikTok</td>
              <td style="padding:10px 14px;border:1px solid #374151;text-align:right;color:#6b7280;">${fmtT(ads.tiktok,1)} €</td>
            </tr>
            <tr>
              ...
              <td style="padding:10px 14px;border:1px solid #374151;text-align:right;color:#6b7280;">
                <div style="font-weight:700;color:#e5e7eb;">${fmtT(costoProductosNeto,2)} €</div>
                ${costoRecuperadoGV > 0 ? `<div style="font-size:10px;color:#9ca3af;margin-bottom:6px;">${fmtT(costoProductos,2)}€ bruto − ${fmtT(costoRecuperadoGV,2)}€ (${numDevGV} dev.)</div>` : ''}
                ...
                <div style="margin-top:6px;border-top:1px solid #374151;padding-top:6px;display:flex;flex-direction:column;gap:3px;">
                  ${Object.values(desgloseProd).map(p => `
                    <div style="display:flex;justify-content:space-between;gap:8px;font-size:11px;">
                      ...
                      <span style="color:#6b7280;white-space:nowrap;flex-shrink:0;">${p.uds} uds × ${fmtT(p.costoUnit,2)}€ = <strong style="color:#9ca3af;">${fmtT(p.costoTotal,2)}€</strong></span>
                    </div>
                  `).join('')}
                </div>` : ''}
              </td>
            ...
            <tr>
              ...
              <td style="padding:10px 14px;border:1px solid #374151;text-align:right;color:#6b7280;">${fmtT(mrw,3)} €
            ...
            <tr style="background:#1f2937;">
              ...
              <td style="padding:10px 14px;border:1px solid #374151;text-align:right;color:#6b7280;">${fmtT(logistica,4)} €
            ...
            <tr>
              ...
              <td style="padding:10px 14px;border:1px solid #374151;text-align:right;color:#6b7280;">${fmtT(fijoXTienda,5)} €
            ...
            <tr style="background:rgba(59,130,246,.08);">
              ...
              <td style="padding:10px 14px;border:1px solid #bfdbfe;">
                <div onclick="abrirModalGastoTienda('shopify','${store.domain}','${mes}',${shopify})"
                  ...
                  <span style="font-size:13px;font-weight:600;color:#2563eb;">${fmtT(shopify,6)}</span>
                  <span style="font-size:11px;color:#93c5fd;">€ ✏️</span>
                </div>
              </td>
            </tr>
            ${(gastosExtras[store.domain]||[]).map((g, gi) => `
            <tr style="background:rgba(59,130,246,.08);">
              ...
              <td style="padding:7px 14px;border:1px solid #bfdbfe;display:flex;align-items:center;gap:6px;">
                <div onclick="abrirModalGastoTienda('extra','${store.domain}','${mes}',${g.valor||0},'${g.id}')"
                  ...
                  onmouseover="this.style.borderColor='#93c5fd'" onmouseout="this.style.borderColor='transparent'">
                  <span style="font-size:13px;font-weight:600;color:#2563eb;">${fmtT(g.valor||0, 7+gi)}</span>
                  <span style="font-size:11px;color:#93c5fd;">€ ✏️</span>
                </div>
